...
function CoordPopup({ info, onClose }) {
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }}
      style={{ position: 'absolute', bottom: '24px', left: '24px', zIndex: 20, padding: '16px 20px', minWidth: '200px' }}
...
function ExplainPanel({ text, onClose, loading }) {
  return (
    <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }}
      style={{ margin: '16px 0', padding: '20px', background: 'rgba(255, 0, 127, 0.05)', border: '1px solid var(--aurora-magenta-dim)', borderRadius: '12px', position: 'relative' }}>
      <button onClick={onClose} style={{ position: 'absolute', top: '12px', right: '12px', background: 'none', border: 'none', color: 'var(--text-muted)', cursor: 'pointer', padding: '4px' }}>✕</button>
      <div style={{ color: 'var(--aurora-magenta)', fontSize: '11px', fontFamily: 'var(--font-mono)', marginBottom: '12px', letterSpacing: '0.1em', display: 'flex', alignItems: 'center', gap: '6px' }}>
      ...
      {loading ? (
        ...
        <p style={{ color: 'var(--text-main)', fontSize: '13px', lineHeight: 1.7, margin: 0, whiteSpace: 'pre-wrap', fontFamily: 'var(--font-display)' }}>{text}</p>
      )}
    </motion.div>
  );
}
...
export default function DashboardPage() {
  ...
  const handleChatYearChange = useCallback((year) => {
    const idx = timeSteps.findIndex(t => t.year === year);
    if (idx !== -1) setTimeIdx(idx);
  }, [timeSteps]);

  const handleChatVariableChange = useCallback((variable) => {
    const valid = ['temperature', 'precipitation', 'wind'];
    if (valid.includes(variable)) setSelVar(variable);
  }, []);
    ...
    try {
      ...
      if (heatData) {
        ...
        stats.forEach(([label, val], i) => {
          ...
          if (i % 2 === 0) { doc.setFillColor(10, 10, 10); doc.rect(x - 2, y - 6, 267, 9, 'F'); }
      ...
      if (heatData?.grid) {
        ...
        doc.setLineWidth(0.2); doc.setDrawColor(...g); doc.line(15, 47, 282, 47);
        ...
        for (let i = 0; i < pts.length && row < 40; i += step, row++) {
          ...
          doc.text(String(p.lat), colX[0], y); doc.text(String(p.lon), colX[1], y);
          doc.text(String(p.value), colX[2], y); doc.text(String(p.norm?.toFixed(3)), colX[3], y);
        }
      }
      ...
      if (explanation) {
        ...
        doc.setDrawColor(...g); doc.line(15, 32, 282, 32);
        doc.setFont('helvetica', 'normal'); doc.setFontSize(10); doc.setTextColor(200, 200, 200);
        doc.text(doc.splitTextToSize(explanation, 265), 15, 42);
      }

      const totalPages = doc.internal.getNumberOfPages();
      for (let i = 1; i <= totalPages; i++) {
        doc.setPage(i); doc.setFontSize(8); doc.setTextColor(50, 50, 50);
  ...
  const currentContext = {
    ...
    stats: heatData ? { mean: heatData.vmean, min: heatData.vmin, max: heatData.vmax, units: heatData.units } : null,
  };

  return (
    <div className="noise" style={{ minHeight: '100vh', display: 'flex', flexDirection: 'column', background: 'var(--bg-void)', overflowY: 'auto', overflowX: 'hidden', position: 'relative' }}>
      <div className="aurora-bg" />
      <Navbar region={region} onRegion={setRegion} />

      {/* ── Top action bar ── */}
      <div className="glass-panel" style={{ marginTop: '80px', marginLeft: '20px', marginRight: '20px', display: 'flex', alignItems: 'center', gap: '12px', padding: '10px 20px', borderRadius: '12px', zIndex: 10, flexShrink: 0, flexWrap: 'wrap', border: '1px solid var(--border-subtle)' }}>
        ...
        {hasFile && <>
          <button className="btn-aurora" style={{ fontSize: '12px', padding: '6px 16px', display: 'flex', alignItems: 'center', gap: '6px' }} onClick={() => setCompare(true)}>
            <span style={{ fontSize: '14px' }}>⊞</span> Compare Matrix
          </button>
          <button className="btn-aurora" style={{ fontSize: '12px', padding: '6px 16px' }} onClick={() => setCities(true)}>◎ City Compare</button>
          <button className="btn-aurora" style={{ fontSize: '12px', padding: '6px 16px', opacity: pdfLoading ? 0.6 : 1 }} onClick={exportPDF} disabled={pdfLoading}>
            {pdfLoading ? '⏳ PDF…' : '↓ Export PDF'}
          </button>
          <button className="btn-aurora" style={{ fontSize: '12px', padding: '6px 16px', borderColor: showExplain ? 'var(--aurora-magenta)' : undefined, background: showExplain ? 'var(--aurora-magenta-dim)' : undefined, color: showExplain ? 'var(--aurora-magenta)' : undefined }}
            onClick={showExplain ? () => setShowExplain(false) : handleExplainAI}>
            🧠 {showExplain ? 'Hide Explain' : 'Explain AI'}
          </button>
        </>}

        <button className="btn-aurora" style={{ fontSize: '12px', padding: '6px 16px', borderColor: showChat ? 'var(--aurora-cyan)' : undefined, background: showChat ? 'var(--aurora-cyan-dim)' : undefined, color: showChat ? 'var(--aurora-cyan)' : undefined }}
        ...
        <div style={{ marginLeft: 'auto', display: 'flex', alignItems: 'center', gap: '8px' }}>
          ...
          <input ref={fileRef} type="file" accept=".nc" style={{ display: 'none' }} onChange={e => handleFile(e.target.files[0])} />
        </div>
      </div>

      <div style={{ flex: 1, position: 'relative', marginTop: '16px', marginLeft: '20px', marginRight: '20px', marginBottom: '20px' }}>
        {/* Background map */}
        <div style={{ position: 'absolute', top: 0, left: 0, right: 0, bottom: 0, borderRadius: '16px', overflow: 'hidden', zIndex: 1, border: '1px solid var(--border-subtle)', boxShadow: '0 0 40px rgba(0,0,0,0.5)' }}>
          {is3D ? (
            <Globe3D data={showHeatmap ? filteredData : null} colorScale={colorScale} onPointClick={handlePointClick} autoRotate={!playing && timeIdx === 0} />
          ) : (
            <Map2D data={showHeatmap ? filteredData : null} colorScale={colorScale} onPointClick={handlePointClick} />
          )}
          <div style={{ position: 'absolute', inset: 0, background: 'radial-gradient(circle at center, transparent 40%, rgba(11,11,19,0.4) 100%)', zIndex: 2, pointerEvents: 'none' }} />
        </div>
        ...
        <PanelGroup direction="horizontal" style={{ height: '100%', position: 'relative', zIndex: 10 }}>
          ...
          {/* ── LEFT PANEL ── */}
          <Panel defaultSize={25} minSize={20} maxSize={35} style={{ opacity: hasFile ? 1 : 0.25, pointerEvents: hasFile ? 'auto' : 'none' }}>
            <div className="glass-panel" style={{ height: '100%', overflowY: 'auto', padding: '20px', display: 'flex', flexDirection: 'column', gap: '24px', borderRadius: '16px', border: '1px solid var(--border-subtle)', background: 'rgba(11, 11, 19, 0.65)' }}>
              ...
              <Section title="01 · VIEW MODE">
                <div style={{ display: 'flex', gap: '8px', background: 'rgba(0,0,0,0.3)', padding: '4px', borderRadius: '8px', border: '1px solid var(--border-subtle)' }}>
                  {['3D Globe', '2D Map'].map((m, i) => (
                    <button key={m} onClick={() => setIs3D(i === 0)} style={{ flex: 1, padding: '8px', borderRadius: '6px', cursor: 'pointer', border: 'none', background: (i === 0) === is3D ? 'var(--aurora-cyan-dim)' : 'transparent', color: (i === 0) === is3D ? 'var(--aurora-cyan)' : 'var(--text-muted)', fontSize: '12px', fontFamily: 'var(--font-display)', fontWeight: 600, transition: 'all 0.2s' }}>{m}</button>
                  ))}
                </div>
              </Section>
              ...
              <Section title="02 · VARIABLE">
                <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
                  {['temperature', 'precipitation', 'wind'].map(v => {
                    const meta = VAR_META[v], active = v === selVar;
                    return (
                      <button key={v} onClick={() => setSelVar(v)} style={{ padding: '12px 14px', borderRadius: '8px', cursor: 'pointer', textAlign: 'left', border: `1px solid ${active ? 'var(--aurora-cyan)' : 'var(--border-subtle)'}`, background: active ? 'var(--aurora-cyan-dim)' : 'rgba(0,0,0,0.2)', color: active ? '#fff' : 'var(--text-muted)', fontSize: '13px', fontFamily: 'var(--font-display)', transition: 'all 0.2s', display: 'flex', alignItems: 'center', gap: '12px', boxShadow: active ? '0 0 15px rgba(0,240,255,0.15)' : 'none' }}>
              ...
              <Section title="03 · YEAR">
                <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '12px', alignItems: 'baseline' }}>
                  <span style={{ fontFamily: 'var(--font-mono)', fontSize: '12px', color: 'var(--text-muted)' }}>{timeSteps[0]?.year || '—'}</span>
                  <span style={{ fontFamily: 'var(--font-display)', fontSize: '28px', fontWeight: 800, color: '#fff', textShadow: '0 0 20px var(--aurora-cyan-dim)' }}>{yearLabel || '—'}</span>
                  <span style={{ fontFamily: 'var(--font-mono)', fontSize: '12px', color: 'var(--text-muted)' }}>{timeSteps[timeSteps.length - 1]?.year || '—'}</span>
                </div>
                <input type="range" className="aurora-slider" min={0} max={Math.max(0, timeSteps.length - 1)} value={timeIdx} onChange={e => setTimeIdx(Number(e.target.value))} disabled={!timeSteps.length} />
              </Section>
                ...
                {trendData.length > 1 ? (
                  <ResponsiveContainer width="100%" height={100}>
                    <AreaChart data={trendData} margin={{ top: 10, right: 0, left: -25, bottom: 0 }}>
                      ...
                      <XAxis dataKey="year" tick={{ fill: 'var(--text-muted)', fontSize: 10, fontFamily: 'var(--font-mono)' }} interval={Math.floor(trendData.length / 4)} axisLine={false} tickLine={false} />
                      ...
                      <Area type="monotone" dataKey="value" stroke="var(--aurora-cyan)" strokeWidth={2} fillOpacity={1} fill="url(#tg)" activeDot={{ r: 4, fill: 'var(--aurora-cyan)', stroke: '#fff' }} />
                    </AreaChart>
                  </ResponsiveContainer>
                ) : (
                  <div style={{ height: '85px', display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#333', fontSize: '12px' }}>Load data to see trend</div>
                )}
              </Section>
              ...
              <Section title="05 · COLOR SCALE">
                ...
                <ColorBar scaleName={colorScale} vmin={heatData?.vmin} vmax={heatData?.vmax} units={heatData?.units} />
              </Section>
                ...
                {heatData ? (
                  <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '10px' }}>
                    {[
                      ...
                      <div key={s.label} className="glass-panel" style={{ background: 'rgba(0,0,0,0.3)', border: '1px solid var(--border-subtle)', borderRadius: '8px', padding: '12px', boxShadow: 'inset 0 0 10px rgba(0,240,255,0.02)' }}>
                        <div style={{ fontSize: '11px', color: 'var(--text-muted)', fontFamily: 'var(--font-mono)', marginBottom: '4px', letterSpacing: '0.05em' }}>{s.label}</div>
                        <div style={{ fontSize: '16px', fontWeight: 800, color: 'var(--aurora-cyan)', fontFamily: 'var(--font-display)', filter: 'drop-shadow(0 0 5px rgba(0,240,255,0.2))' }}>{s.value}</div>
                      </div>
                    ))}
                  </div>
                ) : (
              ...
              <AnimatePresence>
                {showExplain && <ExplainPanel text={explanation} onClose={() => setShowExplain(false)} loading={explainLoading} />}
              </AnimatePresence>
          ...
          {/* ── RIGHT PANEL ── */}
          <Panel>
            <div style={{ height: '100%', display: 'flex', flexDirection: 'column', padding: '12px', gap: '10px', overflow: 'hidden' }}>

              {/* Globe / Map */}
              <div ref={mapRef} className="glass-panel" style={{ flex: '0 0 auto', height: '48%', background: 'rgba(11, 11, 19, 0.4)', border: '1px solid var(--border-subtle)', borderRadius: '12px', overflow: 'hidden', position: 'relative' }}>
                {yearLabel && (
                  <div style={{ position: 'absolute', top: '16px', left: '20px', zIndex: 10, pointerEvents: 'none', fontFamily: 'var(--font-display)', fontSize: '32px', fontWeight: 800, color: 'var(--text-main)', opacity: 0.9, textShadow: '0 0 30px rgba(255,255,255,0.2)' }}>
                    {yearLabel}
                  </div>
                )}
                {selVar && (
                  <div className="glass-panel" style={{ position: 'absolute', top: '16px', left: yearLabel ? '120px' : '20px', zIndex: 10, pointerEvents: 'none', padding: '6px 12px', background: 'rgba(0,0,0,0.6)', border: `1px solid ${varMeta.color}44`, borderRadius: '100px', color: varMeta.color, fontSize: '12px', fontFamily: 'var(--font-mono)', boxShadow: `0 0 15px ${varMeta.color}33`, display: 'flex', alignItems: 'center', gap: '6px' }}>
                ...
                <div style={{ position: 'absolute', top: '16px', right: '16px', zIndex: 10, display: 'flex', gap: '8px', alignItems: 'center' }}>
                  {hasFile && (
                    <button className="glass-panel" onClick={() => setShowHeatmap(h => !h)} style={{ padding: '6px 12px', borderRadius: '100px', cursor: 'pointer', fontSize: '12px', border: `1px solid ${showHeatmap ? 'var(--aurora-cyan)' : 'var(--border-subtle)'}`, background: showHeatmap ? 'var(--aurora-cyan-dim)' : 'rgba(0,0,0,0.5)', color: showHeatmap ? '#fff' : 'var(--text-muted)', fontFamily: 'var(--font-mono)', transition: 'all 0.2s' }}>
                ...
                <AnimatePresence mode="wait">
                  <motion.div key={is3D ? '3d' : '2d'} initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} style={{ width: '100%', height: '100%' }}>
                    {is3D ? (
                      <Globe3D dataPoints={filteredData?.points || []} showHeatmap={showHeatmap && !!heatData} colorScale={colorScale} autoRotate={!playing && !heatData} region={region} onPointClick={handlePointClick} />
                    ) : (
                      <Map2D gridData={filteredData?.grid || null} showHeatmap={showHeatmap && !!heatData} colorScale={colorScale} vmin={heatData?.vmin} vmax={heatData?.vmax} region={region} onPointClick={handlePointClick} />
                    )}
                  </motion.div>
                </AnimatePresence>
              ...
              {/* Timelapse bar */}
              <div className="glass-panel" style={{ background: 'rgba(11, 11, 19, 0.4)', border: '1px solid var(--border-subtle)', borderRadius: '12px', padding: '14px 20px', display: 'flex', alignItems: 'center', gap: '16px', flexShrink: 0 }}>
                <span style={{ color: 'var(--text-muted)', fontSize: '11px', fontFamily: 'var(--font-mono)', whiteSpace: 'nowrap', letterSpacing: '0.1em' }}>TIMELAPSE</span>
                <button onClick={togglePlay} disabled={!hasFile || timeSteps.length < 2} style={{ width: '32px', height: '32px', borderRadius: '50%', border: '1px solid var(--aurora-cyan)', background: playing ? 'var(--aurora-cyan-dim)' : 'transparent', color: 'var(--aurora-cyan)', fontSize: '12px', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0, opacity: !hasFile ? 0.3 : 1, transition: 'all 0.2s', boxShadow: playing ? '0 0 15px rgba(0,240,255,0.3)' : 'none' }}>
                  {playing ? '⏸' : '▶'}
                </button>
                <button onClick={() => { clearInterval(playRef.current); setPlaying(false); setTimeIdx(0); }} style={{ width: '28px', height: '28px', borderRadius: '50%', border: '1px solid var(--border-subtle)', background: 'transparent', color: 'var(--text-muted)', fontSize: '11px', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>⏹</button>
                <input type="range" className="aurora-slider" min={0} max={Math.max(0, timeSteps.length - 1)} value={timeIdx} onChange={e => setTimeIdx(Number(e.target.value))} disabled={!hasFile} style={{ flex: 1, opacity: !hasFile ? 0.3 : 1 }} />
                <span style={{ fontFamily: 'var(--font-mono)', fontSize: '14px', color: 'var(--text-main)', minWidth: '40px' }}>{yearLabel || '—'}</span>
                <div style={{ display: 'flex', gap: '4px', flexShrink: 0 }}>
                  {[0.5, 1, 2, 4].map(s => (
                    <button key={s} onClick={() => setSpeed(s)} style={{ padding: '4px 8px', borderRadius: '4px', cursor: 'pointer', border: `1px solid ${speed === s ? 'var(--aurora-cyan)' : 'var(--border-subtle)'}`, background: speed === s ? 'var(--aurora-cyan-dim)' : 'transparent', color: speed === s ? '#fff' : 'var(--text-muted)', fontSize: '11px', fontFamily: 'var(--font-mono)', transition: 'all 0.2s' }}>{s}×</button>
                  ))}
                </div>
              </div>

              {/* Time Series */}
              <div className="glass-panel" style={{ flex: 1, background: 'rgba(11, 11, 19, 0.4)', border: '1px solid var(--border-subtle)', borderRadius: '12px', padding: '16px', overflow: 'hidden', minHeight: 0 }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '12px' }}>
                  <span style={{ color: 'var(--aurora-cyan)', fontFamily: 'var(--font-mono)', fontSize: '11px', letterSpacing: '0.1em' }}>TIME SERIES — {varMeta.label || selVar} · {region}</span>
                  {heatData && (
                ...
                {trendData.length > 1 ? (
                  <ResponsiveContainer width="100%" height="85%">
                    <AreaChart data={trendData} margin={{ top: 10, right: 18, left: 0, bottom: 0 }}>
                      ...
                      <Tooltip content={<ChartTip />} cursor={{ stroke: 'var(--aurora-cyan)', strokeWidth: 1, strokeDasharray: '3 3' }} />
                      <ReferenceLine x={curStep?.year} stroke="var(--aurora-magenta)" strokeWidth={1} strokeDasharray="3 3" />
                      <Area type="monotone" dataKey="value" stroke="var(--aurora-cyan)" strokeWidth={2} fill="url(#tsG)" activeDot={{ r: 5, fill: 'var(--aurora-cyan)', stroke: '#fff' }} />
                    </AreaChart>
                  </ResponsiveContainer>
                ) : (
                  <div style={{ height: '100%', display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'var(--text-muted)', fontFamily: 'var(--font-mono)', fontSize: '12px', letterSpacing: '0.1em' }}>
      ...
      <AnimatePresence>
        ...
        {showChat && (
          <AIChatPanel
            ...
            onRegionChange={handleChatRegionChange}
            onYearChange={handleChatYearChange}
            onVariableChange={handleChatVariableChange}
          />
        )}
      </AnimatePresence>
    </div>
